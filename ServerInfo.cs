...
namespace Oxide.Plugins
{
	...
	public sealed class ServerInfo : RustPlugin
    {
        ...
        private const string PinkCrystal = "https://i.imgur.com/uBDMCqv.png";
        private const string BlueCrystal = "https://i.imgur.com/yzLjKvG.png";
		...
		private void OnServerInitialized()
		{
            ...
            {
                ImageLibrary.Call("AddImage", PinkCrystal, "pinkCrystal");
                ImageLibrary.Call("AddImage", BlueCrystal, "blueCrystal");
            }
		...
		private void SendUI(BasePlayer player, CuiElementContainer container)
		{
		...
		private string AddMainPanel(CuiElementContainer container)
		{
			...
			var mainPanel = new CuiPanel
			{
                ...
                Image = { Color = ColorExtensions.ToRustFormatString(backgroundColor), Sprite = "Assets/Content/UI/UI.Background.Tile.psd", Material = "assets/content/ui/uibackgroundblur.mat", FadeIn = 0.5f },
                //FadeOut = 0.4f,
                CursorEnabled = true,
				RectTransform =
				{
		...
		private CuiElement CreateImage(string panelName, ImageSettings settings)
		{
			var element = new CuiElement();
			var image = new CuiRawImageComponent
			{
				//Url = settings.Url,
                Png = (string)ImageLibrary.Call("GetImage", settings.Url),
		...
		private string CreateTab(HelpTab helpTab, CuiElementContainer container, string mainPanelName, int pageIndex)
		{
		...
		private string CreateTabPanel(CuiElementContainer container, string mainPanelName, string hexColor)
		{
			Color backgroundColor;
		...
		private string CreateTabContentPanel(CuiElementContainer container, string mainPanelName, string hexColor)
		{
			Color backgroundColor;
			ColorExtensions.TryParseHexString(hexColor, out backgroundColor);
		...
		private CuiLabel CreateHeaderLabel(HelpTab helpTab)
		{
			return new CuiLabel
			{
				RectTransform =
		...
		private CuiButton CreateCloseButton(string mainPanelName, string hexColor)
		{
			Color color;
		...
		private CuiButton CreateHelpButton()
		{
			Color color;
		...
		private CuiLabel CreateTextLineLabel(HelpTab helpTab, float firstLineMargin, float textLineHeight, int textRow,
			string textLine)
		{
			var textLineLabel = new CuiLabel
			{
				RectTransform =
				{
					...
					AnchorMax = "0.95 " + (firstLineMargin - textLineHeight * textRow)
				},
				Text =
				{
		...
		private CuiButton CreatePrevPageButton(string mainPanelName, int pageIndex, string hexColor)
		{
			Color color;
			ColorExtensions.TryParseHexString(hexColor, out color);
			return new CuiButton
			{
				...
				RectTransform =
				{
                    AnchorMin = "0.523 0.01",
                    AnchorMax = "0.749 0.07"
                },
				Text =
				{
		...
		private CuiButton CreateNextPageButton(string mainPanelName, int pageIndex, string hexColor)
		{
			Color color;
			ColorExtensions.TryParseHexString(hexColor, out color);
			return new CuiButton
		...
		private void AddNonActiveButton(
			int tabIndex,
		...
		private string AddActiveButton(
			int activeTabIndex,
			HelpTab activeTab,
			CuiElementContainer container,
			string mainPanelName)
		...
		private CuiButton CreateTabButton(int tabIndex, HelpTab helpTab, Color color)
		{
