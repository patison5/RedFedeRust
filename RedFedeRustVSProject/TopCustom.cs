...
namespace Oxide.Plugins
{
    ...
    class TopCustom : RustPlugin
    {
        ...
        void showMyCustomTop(BasePlayer player, string cmd, string[] Args)
        {
            ...
            // var tmpPVPKills = (List<PlayerData>)getSortedPlayersTop("PVPKills");
            SendReply(player, $"<color=#FFEB3B>####### Top 1 #######</color>");

            PlayerData tmp1 = playersData.OrderByDescending(x => x.PVPKills).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Убийств:</color> {tmp1.nickname}");

            tmp1 = playersData.OrderByDescending(x => x.ResoursesCollectedByHads).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Собрано ресурсов:</color> {tmp1.nickname}");

            tmp1 = playersData.OrderByDescending(x => x.Reputation).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Репутация:</color> {tmp1.nickname}");

            tmp1 = playersData.OrderByDescending(x => x.explosionMade).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Взрывов сделано:</color> {tmp1.nickname}");

            tmp1 = playersData.OrderByDescending(x => x.radHousesLooted).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Радиактивных домов залутано:</color> {tmp1.nickname}");

            tmp1 = playersData.OrderByDescending(x => x.shootDistance).FirstOrDefault();
            SendReply(player, $"<color=#FFEB3B>Самый дальний выстрел:</color> {tmp1.nickname}");
        ...
        private List<BasePlayer> getPlayersListForTopSorted (string sortKey, List<BasePlayer> playersToBeSorted) {

            return (from x in playersToBeSorted orderby Convert.ToInt32(getSinglePlayersTopPositionByKey(x, sortKey, true)) select x).ToList();
        }
 
        private string getSinglePlayersTopPositionByKey(BasePlayer player, string sortKey, bool flag = false) {
            string result = "";
            int length = 1;
            int playerTopPos = -1;
            ...
            switch (sortKey) {
                case "Gathering":
                    ...
                    if (!flag)
                        ...
                        result = $"{ (playerTopPos == -1) ? "10000000" : playerTopPos.ToString() }";

                break;

                case "PVP_KD":
                    // var tmpTop2 = playersData.OrderByDescending(x => (float)x.PVPKills - x.PVPDeath);
        ...
        private Dictionary<string, string> ReturnPlayersStatisticsForTop(BasePlayer player)
        {
            ...
            var statistics = new Dictionary<string, string>() {
                { "Kills",              dat.PVPKills.ToString() },
                { "Deaths",             dat.PVPDeath.ToString() },
                { "K/D",                ((int)(100.0 * dat.PVPKills / ((dat.PVPDeath == 0) ? 1 : dat.PVPDeath)) / 100.0).ToString() },
