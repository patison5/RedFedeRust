...
namespace Oxide.Plugins
{
    ...
    public class VoiceUI : RustPlugin
    {
        ...
        object OnPlayerVoice(BasePlayer voiceSender, Byte[] data)
        {
            List<Connection> recipients = null;

            if (Net.sv.write.Start())
            {
                Net.sv.write.PacketID(Message.Type.VoiceData);
                Net.sv.write.UInt32(voiceSender.net.ID);
                Net.sv.write.BytesWithSize(data);
                Write write = Net.sv.write;
                recipients = BaseNetworkable.GetConnectionsWithin(voiceSender.transform.position, 100f);
                SendInfo sendInfo = new SendInfo(recipients)
                {
        ...
        {
            player.SendConsoleCommand("chat.add", new object[] { "0", string.Format("<color={0}>{1}</color>: {2}", color, from, message) });
        }
        #endregion
